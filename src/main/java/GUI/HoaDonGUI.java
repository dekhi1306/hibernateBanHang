...
public class HoaDonGUI extends JPanel {
    ...
    private ct_HDBLL ctBLL = new ct_HDBLL();
    ...
    private JButton btnDelete, btnView, btnBack;
...
//        btnBack = new JLabel(new ImageIcon("./src/image/btnBack.png"));
        ...
        btnDelete.addMouseListener(new MouseAdapter() {
            public void mouseClicked(MouseEvent e) {
                if (txtMaHD.getText().equals("")) {
                    JOptionPane.showMessageDialog(null ,"Vui lòng chọn hóa đơn cần xóa !!!", "Warning", 
                            JOptionPane.WARNING_MESSAGE);
                    return;
                }
                int i = JOptionPane.showConfirmDialog(null, "Xác nhận xóa", "Alert", JOptionPane.YES_NO_OPTION);
                if (i == 0) {
                    ctBLL.delete(txtMaHD.getText());
                    hdBLL.delete(txtMaHD.getText());
                    JOptionPane.showMessageDialog(null ,"Xóa hóa đơn thành công !!!", "Success", 
                            JOptionPane.INFORMATION_MESSAGE);
                    cleanView();
                    tbl.clearSelection();
                    outModel(model, (ArrayList<hoadon>) hdBLL.getList());
                }
            }
        });
        ...
        btnView.addMouseListener(new MouseAdapter() {
            ...
            public void mouseClicked(MouseEvent e) {
                if (txtMaHD.getText().equals("")) {
                    JOptionPane.showMessageDialog(null ,"Vui lòng chọn hóa đơn cần xem !!!", "Warning", 
                            JOptionPane.WARNING_MESSAGE);
                    return;
                }
...
//        /******************************************/
    ...
    public void search() {
        ...
        outModel(model, hdBLL.search(mm, yyyy, max, min, maHD));
    }
    ...
    public void outModel(DefaultTableModel model, ArrayList<hoadon> hd) {     //xuat tu arraylist len table
        Vector data;
        model.setRowCount(0);
        for (hoadon h : hd) {
            data = new Vector();
    ...
    {
        if (hdBLL.getList() == null) {
            hdBLL.list();
        }
        ArrayList<hoadon> hd = (ArrayList<hoadon>) hdBLL.getList();
        model.setRowCount(0);
        outModel(model, hd);
    }
